...
function normalizeFMP(ticker, d, errors) {
  ...
  // FCF normalizado: mediana de los años disponibles, para detectar un año atipico
  // (por ejemplo un pago extraordinario que hunde el flujo de un ejercicio puntual)
  let fcfMedian = null;
  const fcfSeries = (d.cash || []).map((x) => num(x.freeCashFlow)).filter((v) => v !== null);
  if (fcfSeries.length >= 3) {
    const s = fcfSeries.slice().sort((x, y) => x - y), m = Math.floor(s.length / 2);
    fcfMedian = s.length % 2 ? s[m] : (s[m - 1] + s[m]) / 2;
  }

  // Recompra / dilucion: variacion anualizada de las acciones en circulacion
  let shsCagr = null, shsLatest = null;
  const shsSeries = incSeries.map((x) => num(x.weightedAverageShsOut)).filter((v) => v !== null && v > 0);
  if (shsSeries.length >= 1) shsLatest = shsSeries[0];
  if (shsSeries.length >= 2) {
    const nw = shsSeries[0], od = shsSeries[shsSeries.length - 1], ys = shsSeries.length - 1;
    if (nw > 0 && od > 0 && ys > 0) shsCagr = Math.pow(nw / od, 1 / ys) - 1;
  }
  ...
  const o = {
    ...
    sharesOutstanding: shsLatest, sharesCAGR: shsCagr, freeCashFlowMedian: fcfMedian,
